...
useEffect(() => {
  ...
  const applyDateFilter = () => {
    ...
    setCalendarDropdownOpen(false); // Close the date dropdown
  ...
  setFilterDropdownOpen(false);
  setCalendarDropdownOpen(false); // Force close both dropdowns
  ...
  const fetchTransactions = useCallback(async (startDate, endDate) => {
    try {
      ...
      const response = await fetch("https://prod-erp.nifty10.in/user/transactions", {
        method: "POST",
  ...
  return (filteredTransactions.length > 0 ? filteredTransactions : transactions).length > 0 ? (
          ...
          <div className="profile-section">
            ...
            <div className="statistics-container">
              <Card
                ...
                color="Bullish"
                data={`₹${userDetails?.investedMoney
              ...
              <Card
                ...
                color="Bearish"
                data={`₹${userDetails?.points ? userDetails.points.toFixed(2) : "0.00"
                  }`}
              />
            </div>
          </div>
    ...
    <div className="loading-container">
      <p className="loading-user-data">Transaction Data Fetching</p>
    </div>
  );
};
