          ...
          <div key={userId} className="user-bids-user-card">
            ...
            <div className="user-bids-grid">
              {filteredBids.map((bid, index) => {
                const showBBIcon = bid.listBidCompany?.some(
                  (c) => c.specialCompany && c.liveBB
                );
                ...
                return (
                  <div className="user-bids-card" key={index}>
                    ...
                    <div className="user-bids-details">
                      <p className="bid-test-value">
                        <strong>Entry Fee:</strong>{" "}
                        {bid.freeBid ? (
                          <>
                            <del>₹{bid.paidPoint || "NA"}</del>{" "}
                            <span className="user-bids-free">Free</span>
                          </>
                        ) : (
                          <span>₹{bid.paidPoint || "NA"}</span>
                        )}
                      </p>
                      <p className="bid-test-value">
                        <strong>Points:</strong> {bid.totalPointCount?.toFixed(2) || "0.00"}
                      </p>
                      <p className="bid-test-value">
                        <strong>Won:</strong> ₹{bid.winningPoint?.toFixed(2) || "0.00"}
                      </p>
                    </div>
                    ...
                    <div className="user-bids-companies">
                      ...
                      <div className="user-bids-company-list">
                        {bid.listBidCompany?.map((company, i) => {
                          const isBB = company?.specialCompany && company?.liveBB;

                          return (
                            <div
                              key={i}
                              className={`company-tag user-bids-company-tag ${company?.specialCompany
                                  ? filterType === "Bullish"
                                    ? "big-bull-tag"
                                    : filterType === "Bearish"
                                      ? "big-bear-tag"
                            ...
                            >
                              {isBB && (
                                <span className="company-bb-icon" title="BigBull/BigBear">
                                  BB
                                </span>
                              )}
                              <span
                                className={`company-dot user-bids-dot ${company.companyStatus === "BULLISH"
                                    ? "bullish-dot"
                                    : "bearish-dot"
                                  }`}
                              ></span>
